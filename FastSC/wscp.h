...
void WSCP::set_param(int flag)
{
    ...
    //cout << "ratio " << ratio << " ratioType1 " << ratioType1 << " ratioType2 " << ratioType2 << " ratioType3 " << ratioType3 << " ratioType4 " << ratioType4 << endl;
...
void WSCP::local_search1()
{
    ...
    //if (ratioType3 && best_cost != INT_MAX)
    ...
    //printf("best_cost %d cur_cost %d %d best_count %d set_count %d %d \n", best_cost, cur_cost, cur_cost - best_cost, best_count, set_count, set_count-best_count);
...
void WSCP::local_search()
{
    ...
    //printf("flag4 %d flag5 %d tabu_length %d weight1 %lf weight2 %lf novelty_p %f bms_t %d\n", flag4, flag5, tabu_length, weight1, weight2, novelty_p, bms_t);
    //cout << "bms_t " << bms_t << " tabu_length " << tabu_length << " weight1 " << weight1 << " weight2 " << weight2 << " novelty_p " << novelty_p << endl;
        ...
        for (step = 0; step < max_step; ++step)
        {
            ...
            if (uncover_stack_fill_pointer == 0)
            {
                ...
                else if (cur_cost < best_cost)
                {
                    ...
                    //cout << cur_cost << " " << best_time << " " << set_count << " " << OutterDepth_new << " "<< out_nonimpr_new << endl;
                    gap = gap * 2;
                    gap = std::min(gap,gap_max);
                    out_nonimpr = 0;
                    out_nonimpr_new = 0;
                ...
                if (cur_cost + cost[best_set] + gap >= best_cost || flag_mode1)
                {
                    ...
                    else if (compare1(-score[flip_set], cost[flip_set], score[best_set], cost[best_set]) < 0 || flag_1)
                    {
                        ...
                        if (flip_set == -1 || best_set == -1) {
                            //cout << "333 " << flip_set << " " << best_set << endl;
                            return;
                        }
                        flip(flip_set);
                        flip(best_set);
